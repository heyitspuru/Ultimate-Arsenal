...
function FillDrill({ pattern, onNext, onReroll }: { pattern: Pattern; onNext: () => void; onReroll: () => void }) {
  ...
  return (
    <>
      ...
      <pre className="mt-4 overflow-x-auto rounded-xl border border-border bg-card p-5 font-mono text-caption leading-loose">
        {lines.map((l, i) => {
          if (!l.blank) {
            return (
              <div key={i}>
            ...
            <div key={i} className="flex items-center gap-1.5">
              ...
              {shown ? (
                ...
                <input
                  ...
                  spellCheck={false}
                  placeholder="…"
                  className={`w-full max-w-md rounded border bg-background px-2 py-0.5 font-mono text-caption outline-none transition-colors ${
                    checked
                      ? ok
                        ? "border-foreground"
