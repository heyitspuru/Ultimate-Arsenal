...
**Not** for non-contiguous picks (→ DP/backtracking) or when order can be broken (→ sort / two pointers).
...
=== "Java"
    ...
    int slidingWindow(int[] nums, int k) {
        ...
        for (int right = 0; right < nums.length; right++) {
            sum += nums[right];                 // grow by right edge
            while (windowInvalid(sum, k)) {     // shrink until valid
                sum -= nums[left];
                left++;
            }
...
- Forgetting to update the summary on *both* edges.
- Off-by-one on length (`right - left + 1`).
- Fixed-size windows just pop the element leaving at `right - k`.
